...
if __name__ == "__main__":
    ...
    print('loading precomputed segmentation network')
    ...
    for i in tqdm(range(img_all.shape[0])):
        with torch.no_grad():
            with torch.cuda.amp.autocast():
                ...
                img_fix = F.grid_sample(img_all[i:i+1].cuda().half(),affine2[:1])
                feat = resnet(img_fix)
    ...
    print('setting up registration network')
    ...
    for repeat in range(2):
            ...
            for i in range(half_iterations):
                ...
                with torch.no_grad():
                    with torch.cuda.amp.autocast():
                        ...
                        nnunet_fix = F.grid_sample(input_fix.cuda().half(),affine.half(),padding_mode='border')
                        nnunet_fix2 = F.grid_sample(input_fix2.cuda().half(),affine.half(),padding_mode='border')
                        label_fix = F.grid_sample(label0.half(),affine.half(),padding_mode='border')


                        keypts_fix = weighted_sample(F.interpolate(nnunet_fix2,scale_factor=2,mode='trilinear').argmax(1),2*512,H,W,D)


                        A2 = (torch.randn(3,4)*.035+torch.eye(3,4)).cuda()
                        affine = F.affine_grid(A2.unsqueeze(0),(1,1,H//2,W//2,D//2))
                        nnunet_mov = F.grid_sample(input_mov.cuda().half(),affine.half(),padding_mode='border')
                        nnunet_mov2 = F.grid_sample(input_mov2.cuda().half(),affine.half(),padding_mode='border')
                        label_mov = F.grid_sample(label1.half(),affine.half(),padding_mode='border')
        ...
        #            input = torch.cat((nnunet_fix,nnunet_mov),1).cuda()
                    ...
                    loss = loss2+loss1
                run_dice[i] = (dice_coeff(label_fix.argmax(1).squeeze().contiguous(),warped.argmax(1).squeeze().contiguous(),14).mean())
        ...
        torch.save({'resnet':resnet.state_dict(),'heatmap':heatmap.state_dict(),'regressor':regressor.state_dict(),\
                    'unet_model':unet_model.state_dict()},task_name+'_complete.pth')
        #
