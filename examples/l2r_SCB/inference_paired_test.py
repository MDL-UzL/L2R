...
if __name__ == "__main__":
    ...
    data_path=os.path.dirname(dataset_filename)
    ...
    for idx in range(img_val.shape[0]):
        ...
        disp_path = os.path.join(out_folder+'/disp_'+str(int(val_list[idx*2].split('_')[1])).zfill(4)+'_'+str(int(val_list[idx*2].split('_')[1])).zfill(4)+'.nii.gz')
        nib.save(nib.Nifti1Image(disp_smooth.permute(0,2,3,4,1).squeeze(0).cpu().numpy(), np.eye(4)), disp_path)
        logging.info(disp_path+' saved')